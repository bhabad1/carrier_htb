...
expressionMetric.getMericDetails = async (req, res, next) => {
  try {
    ...
    db.query('SELECT * FROM expressions WHERE id = ?', [metricId], (err, results) => {
      if (err) throw err;
      res.json(results[0]);
    });
  } catch (error) {
...
expressionMetric.updateMetric = async (req, res, next) => {
  try {
    ...
    db.query('UPDATE expressions SET experssion_name = ?, formula = ? WHERE id = ?', [name, formula, metricId], (err) => {
      if (err) throw err;
